...
class Usuario
{
    ...
    public function addUsuario($data)
    {
        if (!isset($data['usr_name']) || !isset($data['usr_email']) || !isset($data['usr_pass'])) {
            ...
            try {
                $query = "INSERT INTO persona (nombre_completo, email, contrasena) VALUES ('$usr_name', '$usr_email', '$usr_pass')";
                $result = mysqli_query($this->conn, $query);
    ...
    public function loginUsuario($data)
    {
        if (!isset($data['usr_email']) || !isset($data['usr_pass'])) {
            ...
            if (mysqli_num_rows($result) > 0) {
                $usuario = mysqli_fetch_assoc($result);
                error_log("contrasena: ". $usuario["contrasena"]);
                error_log("usr_pass: ". $usr_pass);
                if (password_verify($usr_pass, $usuario['contrasena'])) {
                    error_log("contraseñas correctas");
                    /*/ Genera una clave de usuario
                    $usr_key = md5($usuario['usr_email'] . time()); 
                    ...
                    */
                    ...
                    if ($result) {
                        ...
                        return json_encode(["success" => [$usuario['id_persona'], $usr_email, $usuario['nombre_completo']/*, $usr_key*/]]);
                    } else {
                        http_response_code(500);
